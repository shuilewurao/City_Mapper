...
#define ZOOM_FACTOR  2/* For zooming on the graphics */
#endif
...
zoom_in(void (*drawscreen) (void)) {
    ...
    if (get_visible_world().area() >= AREA_5) {
        float xcen, ycen;
...
zoom_out(void (*drawscreen) (void)) {
    ...
    if (get_visible_world().area() <= AREA_0) {
        float xcen, ycen;
        xcen = (trans_coord.xright + trans_coord.xleft) / 2;
        ycen = (trans_coord.ybot + trans_coord.ytop) / 2;
...
void draw_surface(const Surface& surface, float x, float y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(x11_state.ctx, surface.getSurface(),
                xworld_to_scrn(x)-12.5, yworld_to_scrn(y)-25);
